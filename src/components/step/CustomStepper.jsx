      ...
      <div className="flex items-center">
        {steps.map((step, i) => (
          <div key={i} className="flex items-center">
            <motion.div
              className={`relative w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 ${i <= currentStep ? "border-[#CC5E00] bg-[#CC5E00]" : "border-[#815a1b] bg-transparent"}`}
              initial={false}
              animate={{
                scale: i === currentStep ? 1.2 : 1,
                backgroundColor: i <= currentStep ? "#CC5E00" : "transparent",
