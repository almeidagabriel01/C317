  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-2 md:px-0 mt-4">
        {items.map((item) => {
          const quantity = beverageQuantities[item.item.ID] || 0;
      ...
      {Object.keys(beverageQuantities).length > 0 && (
        ...
        >
          ...
          <div className="space-y-2 mb-4">
            {items
              .filter(item => beverageQuantities[item.item.ID] > 0)
              .map(item => (
                <div key={item.item.ID} className="flex justify-between text-white">
                  <span>{item.item.Descricao} <span className="text-gray-400">x {beverageQuantities[item.item.ID]}</span></span>
                  <span>{formatCurrency(item.item.Preco * beverageQuantities[item.item.ID])}</span>
                </div>
              ))}
          </div>
            ...
            <span>
              {formatCurrency(
                items.reduce((sum, item) => {
                  return sum + (item.item.Preco * (beverageQuantities[item.item.ID] || 0));
                }, 0)
              )}
            </span>
          </div>
